...
>
> **Modo mantenedora autônoma 2026-06-24 (`roadmap-todo`):** PR #1 (skill + reconciliação) mergeado na `main` (`3984ca1`). A skill `todo-roadmap-pilot` foi promovida a **mantenedora autônoma** e renomeada para **`roadmap-todo`** — passa a revisar/validar/mergear PRs seguros, decidir o próximo trabalho com evidência, registrar ADR e autoevoluir, mantendo a `main` como fonte oficial. Decisão de autonomia registrada em **[ADR 0013](docs/adr/0013-autonomous-maintainer-skill.md)** (autonomia com guardas; auto-merge só de PRs do próprio ciclo, escopo declarado, sem conflito, sem mudança sensível, checks verdes ou reds comprovadamente pré-existentes). **Merge em `main` não dispara deploy** (`deploy.yml` só roda em tags `v*`/manual). Dívida de CI detectada (pré-existente na `main`): `cargo fmt --check` e `Security — cargo audit` vermelhos → itens no `TODO.md`.
>
> **Reconciliação 2026-06-24 (`roadmap-todo`):** sincronizado com `origin/main` @ `bde3288` (2026-06-18). Verificados **82 commits** mergeados desde 2026-05-24: **Docs Tier 2** expandido (GAR-837/840/845/847/850/853/858, PRs #709–#730); **self-service `/v1/me/*` + Compliance LGPD/GDPR** — sessions, api-keys, password, audit, **GAR-885 export (art. 20)**, **GAR-888 anonymize (art. 12)**, GAR-884 delete (PRs #735–#777); **GAR-890 `DELETE /v1/groups/{id}`** (#779); **busca slices 16+17** (GAR-856 #728); **mutation testing** GAR-891 (#780); **deps/RUSTSEC-2026-0182** GAR-894 (#787); e cadência de **health runs 107–145**. **Nenhuma fase virou status** — sem evidência de fechamento total de fase; detalhe operacional em `TODO.md`. Próximo: avaliar fechamento de Docs Tier 2 (§3.8) e reconciliar checkboxes §3.6/§3.8.
>
